...
public class RentalCarDocument {
	...
	public RentalCarDocument(final Rental rental) {
		...
		this.car = ofNullable(rental.getCar()).map(CarDocument::new).orElse(null);
		this.price = ofNullable(rental.getPrice()).map(PriceDocument::new).orElse(null);
		this.costumer = ofNullable(rental.getCostumer()).map(CostumerDocument::new).orElse(null);
