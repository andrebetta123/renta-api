...
public class CostumerDocument {
	...
	public Costumer toDomain() {
		return Costumer.builder().id(this.id).name(this.name).phone(this.phone).driverLicense(this.driverLicense)
				.active(this.active).address(this.address).createdDate(this.createdDate)
				.lastModifiedDate(this.lastModifieldDate).build();
	}
}
